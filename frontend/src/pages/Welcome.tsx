...
export default function Welcome() {
  ...
  const { me } = useAuth();
  const { push } = useToast();
  const { data, loading } = useApiData<WelcomeData>(`/api/g/${gid}/welcome`);
  ...
  const preview = message
    .replace(/\{user\}/g, "@NeuesMitglied")
    .replace(/\{user_name\}/g, "NeuesMitglied")
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      await api.post(`/api/g/${gid}/welcome`, { enabled, channel_id: channelId, message });
      push("ok", "Willkommensnachricht gespeichert.");
    } catch (err) {
  ...
  async function uploadImage(file: File) {
    ...
    setImgBusy(true);
    try {
      const r = await api.upload(`/api/g/${gid}/welcome/upload`, form);
      setImage(r.url);
  ...
  async function applyUrl() {
    ...
    setImgBusy(true);
    try {
      await api.post(`/api/g/${gid}/welcome/image`, { url: u });
      setImage(u);
      setUrlInput("");
  ...
  async function removeImage() {
    setImgBusy(true);
    try {
      await api.post(`/api/g/${gid}/welcome/image`, { url: "" });
      setImage(null);
  ...
  return (
    <>
      <PageHeader title="Willkommensnachrichten" subtitle="Begrüßung neuer Mitglieder" />
      {enabled && !channelId && (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Einstellungen">
          <div className="toggle-card mb-5">
            <span>
              ...
              <span className="block text-sm text-muted">Neue Mitglieder im gewählten Channel begrüßen.</span>
            </span>
        ...
        {/* Live-Vorschau (Discord Welcome Card) */}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="dc-preview">
            ...
            <div className="dc-body">
              ...
              <div className="dc-name">
                NeuesMitglied
                <Icon name="enter" size={14} className="text-[var(--r-epic)]" />
              </div>
              <div className="dc-text whitespace-pre-wrap"><DiscordMarkdown text={preview} /></div>
            </div>
          </motion.div>
          <p className="mt-3 text-[0.8rem] text-faint">
            So sieht die Begrüßung ungefähr aus (echte Werte beim Join).
          </p>
        </Panel>
      </div>

      {/* Banner / Welcome-Bild */}
      <Panel title="Banner / Welcome-Bild">
        <p className="mb-4 text-sm text-muted">
          Optionales Bild, das groß unter der Nachricht angezeigt wird. Per Drag &amp; Drop, Datei-Auswahl
          oder URL.
        </p>
